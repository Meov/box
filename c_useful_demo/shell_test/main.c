	...
	if (!builtin_cmd(argv))
	{
		...
		if (!bg){
			...
			if(waitpid(pid,&status,0) < 0){
				app_error("waitfg: waitpid pid error");
			}
		}
		//      waitfg(pid);
		else
...
void waitfg(pid_t pid)
{
	 ...
	 *  INT #
	 *  (1) CTRL_C发送INT信号给主进程，中断pause状态,进入INT处理函数,该函数把INT信号转发给前台子进程，处理完成后返回上次中断的位置(2)。
	 *  (2) INT中断处理函数返回后，继续执行下一条指令,发现while循环条件依然成立又进入pause.
	 *  (3) 前台子进程被INT信号终止结束生命发送CHLD信号给主进程，中断pause状态，进入CHLD处理函数，该函数回收僵尸进程，删除对应的job,处理完成后返回上次中断的位置。
	 *  (4) CHLD中断返回后，继续执行下一条指令，发现while循环条件不再成立，跳出whlile循环。
	 *
